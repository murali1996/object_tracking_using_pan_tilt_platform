...
const char* const SERIAL_PORT_DEVICE = "/dev/ttyACM3" ;
 ...
 FILE_LOG(logINFO) << "#" << frame << " active: " << cmt.points_active.size()<<"Using " << (int)cmt.center.x<<"--->"<<(int)cmt.center.y << "," << cmt.scale;
       
 FILE *fp;
 fp = fopen("/dev/ttyACM3", "w");

/*
//------------------------------------------- This is for PAN-------------------------------------------
 if ((int)cmt.center.x-320>10){
 	current_error_x=(int)cmt.center.x-320;
 	//sum_error_x=sum_error_x+current_error_x;
	temp_x=100+((int)cmt.center.x-320)*10/320; //(current_error_x-previous_error_x)*5/50; // + (sum_error_x)*1/50000;
	if(temp_x<100 || temp_x>120){ // Can change this too
		fprintf(fp, "100");  
	    FILE_LOG(logINFO) << "100";
	}   
	else{
		fprintf(fp, "%d", temp_x);
		FILE_LOG(logINFO) << temp_x;
	} 
	sleep(.5);
 }
 else if ((int)cmt.center.x-320<-10){
 	current_error_x=(int)cmt.center.x-320;
 	//sum_error_x=sum_error_x+current_error_x;
 	temp_x=200+(320-(int)cmt.center.x)*10/320; // (current_error_x-previous_error_x)*5/50; // + (sum_error_x)*1/50000;
 	if(temp_x<200 || temp_x>220){ // Can change this too
		fprintf(fp, "200"); 
	    FILE_LOG(logINFO) << "200";
	}   
	else{
		fprintf(fp, "%d", temp_x);
		FILE_LOG(logINFO) << temp_x;
...
//------------------------------------------- This is for TILT------------------------------------
 if ((int)cmt.center.y-240>10){ 
	temp_y=300+((int)cmt.center.y-240)*5/240;
	if(temp_y<300 || temp_y>320){ // Can change this too
		fprintf(fp, "300");
	    FILE_LOG(logINFO) << "300";
	}   
	else{
		fprintf(fp, "%d", temp_y); 
		FILE_LOG(logINFO) << temp_y;
	} 
	sleep(.5);
 }
 else if ((int)cmt.center.y-240<-10)	{
 	temp_y=400+(240-(int)cmt.center.y)*5/240;
	if(temp_y<400 || temp_y>420){ // Can change this too
		fprintf(fp, "400");  fprintf(fp, "e");
	    FILE_LOG(logINFO) << "400";
	}   
	else{
		fprintf(fp, "%d", temp_y);  fprintf(fp, "e");
		FILE_LOG(logINFO) << temp_y;
	} 
    sleep(.5);
 }
...
if ((int)cmt.center.x-320>10){
	current_error_x=(int)cmt.center.x-320;
	if ((int)cmt.center.y-240>10)
		temp= 300100+((int)cmt.center.x-320)*5/320 + (current_error_x-previous_error_x)*1/50 + 1000*floor(((int)cmt.center.y-240)*3/240) ; // 5*1000
 	else if ((int)cmt.center.y-240<-10)
 		temp= 400100+ ((int)cmt.center.x-320)*5/320 + (current_error_x-previous_error_x)*1/50 + 1000*floor((240-(int)cmt.center.y)*3/240);
 	fprintf(fp, "%d", temp);
	FILE_LOG(logINFO) << temp;
}
else if ((int)cmt.center.x-320<-10){
	current_error_x=(int)cmt.center.x-320; 
 	if ((int)cmt.center.y-240>10)
		temp= 300200+ (320-(int)cmt.center.x)*5/320 + (current_error_x-previous_error_x)*1/50 + 1000*floor(((int)cmt.center.y-240)*3/240);
 	else if ((int)cmt.center.y-240<-10)
 		temp= 400200+ (320-(int)cmt.center.x)*5/320 + (current_error_x-previous_error_x)*1/50 + 1000*floor((240-(int)cmt.center.y)*3/240);
 	fprintf(fp, "%d", temp);
	FILE_LOG(logINFO) << temp;
 } 
